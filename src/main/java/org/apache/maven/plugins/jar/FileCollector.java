...
final class FileCollector extends SimpleFileVisitor<Path> {
    ...
    Path handleOrphanFiles() {
        if (moduleHierarchy.isEmpty() || packageHierarchy.canSkip(true)) {
    ...
    void writeAllJARs(final ToolExecutor executor) throws IOException {
        for (Archive module : moduleHierarchy.values()) {
            if (!module.canSkip(executor.skipIfEmpty)) {
                executor.writeSingleJAR(this, module);
            }
        }
        // `packageHierarchy` is expected to be empty if `moduleHierarchy` was used.
        if (!packageHierarchy.canSkip(executor.skipIfEmpty || !moduleHierarchy.isEmpty())) {
